...
--    initial = "done",
--    events = {
--        { name = "movesafe", from = {"done", "stopped"} , to = "moving_safe"},
...
state.movement_state.onmovesafe = function(fsm, name, from, to, x, y, theta)
...
state.action_state = machine.create ( {
    ...
    events = {
        ...
        {name = "follow_wall_S1", from = '*', to = "followingwallS1"}---- Deploy arm
...
state.action_state.onmove_S1 = function(fsm, name, from, to, timestamp)
    ...
    move_servo(1, 3000)
    local x, y = get_wpt_coords("S1")
    state.movement_state:movesafe(x, y, config.theta_pince_mur) -- 60° in radians
    ...
    state.movement_state.ondone = function (self, event, from, to)
        ...
        state.movement_state:move_blind(x_dest, y_dest, config.theta_pince_mur - 0.1)
        state.movement_state.ondone = function (self, event, from, to)
